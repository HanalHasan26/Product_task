            ...
            <Card>
                <CardHeader>
                    <H5 attrH5={{ className: "mb-0" }}>
                        <Btn attrBtn={{ color: "link ps-0", onClick: () => setisMutual(!isMutual) }} >{MutualFriends}</Btn>
                    </H5>
                </CardHeader>
                <Collapse isOpen={isMutual}>
                    <CardBody className="social-status filter-cards-view">
                        ...
                        {MutualFriendsData.map((data, i) => (
                            <Media key={i}>
                                <Image attrImage={{ className: "img-50 rounded-circle m-r-15", src: `${data.img}`, alt: "twoImg" }} />
                                <div className={`social-status ${data.status}`}></div>
                                <Media body>
                                    <span className="f-w-500 d-block">{data.name}</span><span className="d-block">{data.email}</span>
                                </Media>
                            </Media>
                        ))}
                    </CardBody>
